      ...
      {phase === "parsing" ? (
        ...
        <div className="pop-in">
          ...
          <details className="mt-4 group">
            <summary className="cursor-pointer select-none text-xs font-medium text-[var(--muted)] hover:text-teal-600 dark:hover:text-teal-400">Raw JSON response</summary>
            <div className="mt-3">
        ...
        <div className="animate-shimmer animate-float relative h-28 w-22 overflow-hidden rounded-lg border border-teal-300/70 bg-gradient-to-b from-teal-50 to-white shadow-lg shadow-teal-500/10 dark:border-teal-800/70 dark:from-teal-950/40 dark:to-zinc-900">
          <div className="space-y-2 p-3">
            {[14, 10, 13, 8, 12, 9].map((w, i) => (
              <div key={i} className="h-1.5 rounded-full bg-teal-200/80 dark:bg-teal-800/70" style={{ width: `${w * 6}%` }} />
            ))}
          </div>
          ...
          {STAGES.map((s, i) => {
            ...
            return (
              <li key={s} className="flex items-center gap-2.5 text-sm">
                <span className={"grid h-5 w-5 shrink-0 place-items-center rounded-full border transition-all duration-300 " + (done ? "border-teal-500 bg-teal-500 text-white" : active ? "border-teal-500 text-teal-600 dark:text-teal-400" : "border-[var(--line)] text-transparent")}>
                  {done ? (
